...
void LLStatBar::drawTicks( F32 min, F32 max, F32 value_scale, LLRect &bar_rect )
{
    ...
    //    return;
    //}
 // [CATWALK:RBN] <BUILD FIX> Check for invalid value_scale
    if (!llfinite(value_scale) || value_scale == 0.f)
    {
        LL_WARNS("STATBAR") << "Invalid value_scale: " << value_scale << " for stat: " << this->getName() << LL_ENDL;
        return;
    }
